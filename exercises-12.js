...
function countProfit(shoppers) {
    ...
    // you can only write your code here!
    if(!!!shoppers || !!!shoppers[0] || !!!shoppers[0].name || !!!shoppers[0].product || !!!shoppers[0].amount) return []
    ...
    shoppers.forEach(function(itemUser){
        for(let i=0;i<listBarang.length;i++){
            let itemBarang = output[i]
            if(itemUser.product == itemBarang.product && (itemBarang.leftOver-itemUser.amount)>=0){
                itemBarang.shoppers.push(itemUser.name)
                itemBarang.leftOver-=itemUser.amount

                listBarang.forEach(function(item){
                    if(item[0] == itemBarang.product){
                        itemBarang.totalProfit+=(itemUser.amount * item[1])
                    }
                })

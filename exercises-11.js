...
function shoppingTime(memberId, money) {
    ...
    if(!!!money || Number(money) < 50000) return 'Mohon maaf, uang tidak cukup'
    ...
    let listProduct = [
        ...
        {
            productId: 'Sweater Uniklooh',
            price: 175000
        },
        {
            productId: 'Casing Handphone',
            price: 50000
        },
    ]
    ...
    while(canBuy && changeMoney > 0){
        for(let i = 0;i<listProduct.length;i++){
            let item = listProduct[i]
            if((changeMoney - item.price) < tempMoney && (changeMoney - item.price) >= 0){
                tempMoney = (changeMoney - item.price)
                tempProductId = item.productId
                posisi = i
            }
        }

        listProduct.splice(posisi, 1)
        
        if(changeMoney != tempMoney){
            changeMoney = tempMoney
            listPurchased.push(tempProductId)
        } else{
            canBuy = false
        }
    }

...
function cariModus(arr) {
    ...
    for(let i=0;i < arr.length;i++){
        ...
        listFrekuensi[listFrekuensi.length] = frekuensi
    }

    frekuensi = listFrekuensi[0]
    for(let i=1;i< listFrekuensi.length;i++){
        if(frekuensi < listFrekuensi[i] ){
            frekuensi = listFrekuensi[i]
            posisi = i
        }
    }
    ...
    return (listFrekuensi.length - 1 > countAngkaSama ? arr[posisi] : -1)
}
